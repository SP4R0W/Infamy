...
export(String, "UZI","M16") var weapon_name = "UZI"
var weapon_mag = 0
...
var whitedeath_can_deal_damage = true
...
func kill(damage: float = 0,type: String = "none"):
	if (!is_dead):
		...
		if (Game.get_skill("mastermind",3,3) != "none" && type == "sniper"):
			...
			elif (Game.get_skill("mastermind",3,3) == "upgraded" && Game.desperado_kill_counter >= 3 && !Game.desperado_active):
				timer.stop()
				Game.desperado_active = true
				Game.desperado_kill_counter = 0
...
func take_damage(damage: float,type: String = "none"):
	if ((type == "shotgun" && Game.get_skill("commando",4,3) != "none") || Game.get_skill("commando",4,3) == "upgraded"):
		health -= (damage * 1.5)
	else:
		health -= damage
