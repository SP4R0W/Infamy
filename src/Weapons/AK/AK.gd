...
var modification_offsets: Dictionary = {
	...
	"default_ak_stock":Vector2(-550,0),
...
func draw_attachments():
	...
	if (attachments["stock"] != ""):
		...
		$Sprite/Stock_attachment.texture = load(Global.weapon_attachment_paths["default_ak_stock"])
		$Sprite/Stock_attachment.offset = modification_offsets["default_ak_stock"]
	...
	if (Game.get_skill("engineer",2,3) != "none"):
		mag_size += 10
