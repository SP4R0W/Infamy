...
func show_panel():
	# Triggered when Interaction raycast collides with object
	has_focus = true
	$Interaction_panel.show()
	$Interaction_panel/VBoxContainer/Interaction_progress.hide()

func hide_panel():
	# Triggered when Interaction raycast no longer collides with object
	has_focus = false
	$Interaction_panel.hide()
...
func _process(delta):
	if (has_focus && can_interact):
		...
		elif (Input.is_action_pressed("interact2") && Game.player_can_interact && has_ammo):
			if (!Game.player_is_interacting):
				for item in Game.player_equipment:
					if (item.has("ammo")):
						if (item[1] >= Game.max_equipment_amounts["ammo"]):
							Game.ui.update_popup("You can't carry more ammo kits!",2)
...
func _on_Interaction_timer_timeout():
	if (Game.player_is_interacting):
		...
		# Only allow use when player has a weapon equipped
		if (action == "use" && Game.player.current_weapon != -1):
			...
			for item in Game.player_equipment:
				if (item.has("ammo")):
					# Update the ammo bag amount in inventory
					var needed = Game.max_equipment_amounts["ammo"] - item[1]
