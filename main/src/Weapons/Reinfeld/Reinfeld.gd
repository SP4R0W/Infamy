...
var modification_offsets: Dictionary = {
	"compensator_shotgun":Vector2(585,0),
	"default_m4_stock":Vector2(-625,0),
	"wide_stock":Vector2(-645,0),
}
	...
	if (!Game.player_is_reloading):
		if (current_mag_size > 0 && firerate_timer.time_left <= 0):
			...
			for x in range(pellet_amount):
				...
				pellet.type = type
				Game.game_scene.add_child(pellet)
				pellet.global_position = Game.player.get_node("Bullet_origin").global_position
		...
		reload_timer.stop()
		anim_player.play("reload_finish")
...
func reload_finish():
	var needed_bullets: int = 0

	if (Game.get_skill("commando",2,3) == "upgraded" && current_mag_size <= (mag_size - 2)):
		needed_bullets = 2
	else:
		needed_bullets = 1
	...
	if (current_ammo_count < needed_bullets):
		...
		current_ammo_count -= needed_bullets
		current_mag_size += needed_bullets
